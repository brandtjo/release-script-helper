...
public class GenerationMenuAddScript extends BasicAddScript {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if(defaultDirectory.isPresent() && editor != null) {
            createAndOpenReleaseScript(defaultDirectory.get(), false, editor.getSelectionModel().getSelectedText(true));
        } else if (defaultDirectory.isPresent()) {
            createAndOpenReleaseScript(defaultDirectory.get(), false, null);
        } else {
            Messages.showErrorDialog("Select a default directory in plugin project settings!", "Script Generation Canceled");
        }
    }
