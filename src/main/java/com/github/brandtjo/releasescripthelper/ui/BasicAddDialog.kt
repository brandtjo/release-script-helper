...
class BasicAddDialog(private val releaseScript: ReleaseScript, private val currentProject: Project) : DialogWrapper(true) {
    ...
    private fun updateUiComponentsByHandBecauseSwingIsStupid() {
        ...
        if(!releaseScript.options.ticketTypes.contains(ticketType.component.selectedItem)) {
            ticketType.component.selectedItem = if (releaseScript.options.ticketTypes.isNotEmpty()) releaseScript.options.ticketTypes.get(0) else "OCT"
        }
        ...
        if(!releaseScript.options.fileEndings.contains(fileEndings.component.selectedItem)) {
            fileEndings.component.selectedItem = if (releaseScript.options.fileEndings.isNotEmpty()) releaseScript.options.fileEndings.get(0) else "sql"
        }
    }
}