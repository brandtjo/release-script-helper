...
public class FileUtil {
    ...
    @Nullable
    public static VirtualFile fromRelativePresentableUrl(@NotNull Project project, @NotNull String relativePresentableUrl) {
    ...
    public static String toRelativePresentableUrl(@NotNull Project project, @NotNull VirtualFile projectChildFile) {
        if(!project.isDefault() && project.getBasePath() != null) {
            VirtualFile projectFile = LocalFileSystem.getInstance().findFileByPath(project.getBasePath());
            if(projectFile != null && projectChildFile.getPresentableUrl().startsWith(projectFile.getPresentableUrl())) {
                return projectChildFile.getPresentableUrl().substring((projectFile.getPresentableUrl() + File.separatorChar).length());
            }
        }
